...
const generatedValue = (parameter = null) => ({
	errorMessage: {
		en_US: null,
	},
	expression: {
...
const ValidationWithProvider = ({validations, builderPages, ...props}) => (
	<FormProvider initialState={{validations, builderPages}}>
		<Validation {...props} />
	</FormProvider>
);
	...
	it('renders parameter field with Date element', () => {
		...
		const {container} = render(
			<ValidationWithProvider
				...
				value={generatedValue({startsFrom: {}})}
			/>
		);
